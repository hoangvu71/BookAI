"""LLM-powered intelligent routing using Vertex AI."""
...
class VertexLLMRouter:
    """Intelligent router using Vertex AI to analyze queries and determine best agents."""
    
    def __init__(self):
        self.logger = structlog.get_logger().bind(component="vertex-llm-router")
        ...
        try:
            if self.settings.google_cloud_project:
                ...
                self.enabled = True
                self.logger.info("LLM router initialized with Vertex AI",
                               project=self.settings.google_cloud_project,
                               model="gemini-1.5-flash")
            else:
                self.enabled = False
                self.logger.warning("LLM router disabled - no Google Cloud project configured")
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model": "gemini-1.5-flash (Vertex AI)" if self.enabled else "keyword-based"
        }